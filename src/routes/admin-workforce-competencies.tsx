...
export const Route = createFileRoute("/admin-workforce-competencies")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "View workforce competency levels across departments and skills.",
      },
    ],
  }),
...
type LevelLabel = "Advanced" | "Proficient" | "Developing" | "Needs Focus";
...
function WorkforceCompetenciesPage() {
  ...
  const departments = useMemo(
    () => [
      "All Departments",
      ...new Set(mockAdminData.competency_heatmap.map((item) => item.department)),
    ],
    [],
  );

  const filteredRows = useMemo(() => {
    const rows = mockAdminData.competency_heatmap.filter((item) =>
      department === "All Departments" ? true : item.department === department,
    );
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, { department: string; items: typeof filteredRows }>();
  ...
  const workforceAverage =
    mockAdminData.competency_heatmap.length > 0
      ? (
          mockAdminData.competency_heatmap.reduce(
            (sum, item) => sum + item.avg_level,
            0,
          ) / mockAdminData.competency_heatmap.length
        ).toFixed(1)
      : "—";
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
                ...
                <p className="mt-3 text-2xl font-extrabold text-destructive">
                  {
                    mockAdminData.overall_skill_gaps.filter(
            ...
            <section className="rounded-xl border border-border bg-card shadow-sm">
              ...
              <div className="divide-y divide-border">
                {grouped.length === 0 ? (
                  ...
                  grouped.map((group) => (
                    <div key={group.department} className="p-5">
                          ...
                          >
                            ...
                            <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, Math.max(0, item.avg_level * 20))}%`,
                                }}
                              />
                            </div>
                          </div>
