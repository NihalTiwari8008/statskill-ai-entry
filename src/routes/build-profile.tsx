...
import { defaultCurrentUserProfile, saveCurrentUserProfile } from "@/lib/current-user";
...
function BuildProfilePage() {
  ...
  const [profile, setProfile] = useState<ProfileData>(() => {
    if (typeof window === "undefined") {
      return {
        name: defaultCurrentUserProfile.name,
        designation: defaultCurrentUserProfile.designation,
        department: defaultCurrentUserProfile.department,
        currentAssignment: defaultCurrentUserProfile.currentAssignment,
        highestQualification: defaultCurrentUserProfile.highestQualification,
        yearsOfExperience: defaultCurrentUserProfile.yearsOfExperience,
        previousTraining: defaultCurrentUserProfile.previousTraining,
      };
    }
    try {
      const raw = window.localStorage.getItem("statskill.currentUserProfile");
      const saved = raw ? JSON.parse(raw) : defaultCurrentUserProfile;
      return { ...defaultCurrentUserProfile, ...saved };
    } catch {
  ...
  const [existingSkills, setExistingSkills] = useState<string[]>(() => {
    if (typeof window === "undefined") return defaultCurrentUserProfile.existingSkills;
    try {
      const raw = window.localStorage.getItem("statskill.currentUserProfile");
      const saved = raw ? JSON.parse(raw) : defaultCurrentUserProfile;
      return Array.isArray(saved.existingSkills) ? saved.existingSkills : defaultCurrentUserProfile.existingSkills;
    } catch {
      return defaultCurrentUserProfile.existingSkills;
    }
  });
  const [workExperience, setWorkExperience] = useState(() => {
    if (typeof window === "undefined") return defaultCurrentUserProfile.workExperience;
    try {
      const raw = window.localStorage.getItem("statskill.currentUserProfile");
      const saved = raw ? JSON.parse(raw) : defaultCurrentUserProfile;
      return typeof saved.workExperience === "string" ? saved.workExperience : "";
    } catch {
      return "";
    }
  });
